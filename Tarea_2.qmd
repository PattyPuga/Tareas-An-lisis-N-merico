...
plt.plot(xnew, L1(xnew), label=r"$L_1(x)$", color="hotpink", linewidth=2)
plt.plot(xnew, L2(xnew), label=r"$L_2(x)$", color="red", linewidth=2)
plt.plot(xnew, L3(xnew), label=r"$L_3(x)$", color="purple", linewidth=2)
plt.plot(xnew, L4(xnew), label=r"$L_4(x)$", color="orange", linewidth=2)
...
plt.figure(figsize=(8,5))
plt.plot(xnew,  p(xnew), label="Polinomio", color="red")
...
p = np.array([2, -7.5, -9.8, -5.2])   # Valores reales
p_star = np.array([2, -8, -10, -5])    # Valores aproximados

# Cálculo de errores
error_real = p - p_star
error_abs = np.abs(error_real)
error_rel = np.abs(error_real) / np.abs(p)

# Mostrar resultados
for i in range(len(p)):
    print(f"Para p = {p[i]} y p* = {p_star[i]}:")
    print(f"Error real: {error_real[i]}")
    print(f"Error absoluto: {error_abs[i]}")
    print(f"Error relativo: {error_rel[i]}\n")
...
# Graficar los errores absolutos y relativos
plt.plot(x_values, error_abs, label='Error absoluto', color='red', marker='o')
plt.plot(x_values, error_rel, label='Error relativo', color='blue', marker='x')

# Configuración de la gráfica
plt.title('Errores Absolutos y Relativos')
...
El polinomio de Lagrange construye un único polinomio global que pasa exactamente por todos los nodos. Al observar las gráficas:

Para muestra 1, la curva de Lagrange logra un ajuste razonable en el centro, pero muestra fluctuaciones notables cerca de los extremos (en días 0 y 28), un efecto conocido como fenómeno de Runge.

Para muestra 2, las oscilaciones son aún más evidentes: aunque el polinomio pasa por todos los puntos, la forma de la curva entre ellos no siempre refleja un comportamiento biológicamente esperado, mostrando picos y valles abruptos.

Muestra 1: hojas jóvenes Las gráficas muestran que:

Las larvas criadas en hojas jóvenes experimentan un aumento rápido de peso hasta aproximadamente el día 10–13, alcanzando un pico de más de 40 mg.

El polinomio de Lagrange a veces genera oscilaciones artificiales, sobre todo en los extremos (día 0 y día 28), que no tienen sentido biológico: por ejemplo, puede sugerir un aumento del peso al final cuando en realidad los datos muestran descenso.

El spline cúbico natural utiliza una serie de polinomios cúbicos por tramos, asegurando suavidad (las derivadas primera y segunda son continuas en los nodos) y evitando oscilaciones exageradas.

Al observar las gráficas:

Para muestra 1, el spline ofrece una curva suave que sigue de forma realista las tendencias generales de los datos, sin sobreajustar las pequeñas variaciones.

Para muestra 2, el spline suaviza las caídas bruscas que el polinomio de Lagrange exagera, mostrando un descenso progresivo más consistente con un proceso biológico de disminución del peso.

Esto resalta que el spline tiene mejor estabilidad numérica y produce resultados más robustos frente a posibles irregularidades locales en los datos.

El spline cúbico natural, al ser una curva suavizada por tramos, refleja mejor las tendencias generales:

Para muestra 1, muestra un crecimiento rápido seguido de una meseta.

Para muestra 2, refleja una caída constante después de un pequeño aumento inicial.

Conclusión clave: Los splines cúbicos son más adecuados porque respetan las tendencias reales sin introducir artefactos matemáticos que podrían llevar a interpretaciones erróneas.
...
El polinomio de Lagrange pasa exactamente por todos los nodos, pero puede mostrar oscilaciones no deseadas entre ellos, sobre todo si los puntos no están equiespaciados o tienen curvaturas locales. En este caso, como hay pocos puntos y la relación entre densidad y temperatura es bastante suave, el polinomio de Lagrange probablemente funciona aceptablemente.

El spline cúbico suaviza las transiciones entre puntos, asegurando continuidad en las derivadas primera y segunda, lo que es físicamente razonable: la densidad del agua no cambia bruscamente con la temperatura.

Comentario: La densidad del agua decrece con el aumento de temperatura porque el líquido se expande ligeramente. Las gráficas reflejan esta relación decreciente.
...
Comentarios: Aproximación de densidad: Ambos métodos deberían dar valores similares para T=63C, ya que este punto cae bien dentro del rango de datos y cerca de uno de los nodos (65∘C).

Derivada (pendiente): Esto nos dice cómo cambia la densidad respecto a la temperatura. Un valor negativo indicará que la densidad está disminuyendo al aumentar la temperatura, lo cual es consistente con el comportamiento físico del agua.

La derivada representa la sensibilidad de la densidad a los cambios de temperatura. Esta información es relevante en aplicaciones de ingeniería, por ejemplo, para calcular expansiones volumétricas o ajustes en sistemas hidráulicos.
:::
